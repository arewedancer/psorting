...
T* divide( T* first, T* last, Compare cmp ) {
    // Move partition key to front.
    std::swap( *first, *choose_partition_key(first,last) );
    // Partition 
    T key = *first;
    //T* middle = std::partition( first+1, last, [=](const T& x) {return asc? x<key: x>key;} ) - 1;
    T* middle = std::partition( first+1, last, [=](const T& x) {return cmp(x, key);} ) - 1;
    if( middle!=first ) {
        // Move partition key to between the partitions
        std::swap( *first, *middle );
    } else {
        // Check if all keys are equal
        //if( last==std::find_if( first+1, last, [=](const T& x) {return asc? key<x : key>x;} ) )
        if( last==std::find_if( first+1, last, [=](const T& x) {return cmp(x,key);} ) )
            return NULL;
    }
    return middle;
}
